...
export class Telegram {
  ...
  constructor() {
    ...
    this.url = "https://tgapp.matchain.io/";
    this.bot = "MatchQuestBot";
  }
  ...
  async runOption() {
    try {
      const sessionList = Helper.getSession("sessions");
      let ctx = "Choose Run Option :\n \n";
      ctx += "1. Match run \n2. One By One run\n \nChoose Run Option : ";

      const mode = await input.text(ctx);
      if (mode == 1 || mode == 2) {
        this.runMode = mode;
      } else {
        console.error("Invalid input, Please try again");
